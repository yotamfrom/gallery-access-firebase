...
function mapCollectionItem(record) {
    const fieldData = record.fieldData || record;
    return {
...
async function createCollection(token, name, description, galleryId) {
...
async function addToCollection(token, collectionId, artworkIds) {
...
async function getGalleryIdFromUid(uid) {
...
async function verifyCollectionOwnership(token, idOrUuid, galleryId) {
    try {
...
async function deleteCollection(token, recordId, collectionUuid) {
    // 1. Clean up items
...
async function removeFromCollection(token, collectionId, workId) {
...
async function updateCollectionItemsOrder(token, items) {
...
async function createShareLink(token, collectionId) {
    ...
    if (!response.ok)
        throw new Error(`Share link creation failed: ${await response.text()}`);
    return { success: true, shareToken };
}
async function getCollectionItems(token, collectionId) {
...
async function getSharedCollection(token, shareToken) {
    ...
    if (!findShareResponse.ok)
        throw new Error('Invalid or expired share link');
    const shareData = await findShareResponse.json();
    ...
    if (!findColResponse.ok)
        throw new Error('Collection no longer exists');
    const findColData = await findColResponse.json();
    ...
    if (artworkIds.length > 0) {
        ...
        if (worksResp.ok) {
            ...
            const artworksMap = new Map(artworksRecords.map((r) => [String(r.fieldData.studio_work_inventory_no_pk), mapArtwork(r)]));
            fullArtworks = rawItems.map((item) => ({
                ...artworksMap.get(String(item.work_id_fk)),
...
async function getLayoutMetadata(token, layout) {
    ...
    if (!response.ok)
        throw new Error(`Metadata failed: ${await response.text()}`);
    const data = await response.json();
    return data.response?.fieldMetaData || [];
}
async function validateGalleryCredentials(token, username, password) {
    console.log('Validating gallery credentials for:', username);
    try {
        const response = await fetch(`https://${FM_HOST}/fmi/data/${FM_API_VERSION}/databases/${FM_DATABASE}/layouts/Galleries/_find`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${token}`,
...
const FM_PASSWORD_SECRET = (0, params_1.defineSecret)('FM_PASSWORD');
...
async function computeHkdf(ikm, salt) {
...
class AuthenticationHelper {
    ...
    async getPasswordAuthenticationKey(username, password, serverBHex, saltHex) {
        if (!this.largeA || !this.k)
            throw new Error('Must call init() first');
...
let FM_PASSWORD = '';
// Token caches
let cachedClarisIdToken = null;
let cachedClarisIdTokenExpiry = 0;
let cachedFmSessionToken = null;
...
async function getClarisIdToken(forceRefresh = false) {
    const now = Date.now();
    if (!forceRefresh && cachedClarisIdToken && now < cachedClarisIdTokenExpiry)
        return cachedClarisIdToken;
    if (!forceRefresh) {
    ...
    const hkdf = await authHelper.getPasswordAuthenticationKey(challengeParams.USER_ID_FOR_SRP, FM_PASSWORD, challengeParams.SRP_B, challengeParams.SALT);
    ...
    if (!respondResponse.ok)
        throw new Error(`RespondToAuthChallenge failed: ${await respondResponse.text()}`);
...
async function getSessionToken(forceRefresh = false) {
    const now = Date.now();
    if (!forceRefresh && cachedFmSessionToken && now < cachedFmSessionTokenExpiry)
        return cachedFmSessionToken;
    if (!forceRefresh) {
    ...
    if (!response.ok)
        throw new Error(`FileMaker Data API session failed: ${response.status} - ${await response.text()}`);
    const data = await response.json();
...
async function withSession(actionFn) {
...
exports.filemakerApi = (0, https_1.onRequest)({
    ...
    try {
        ...
        if (req.query.action === 'proxyImage' || (req.body?.action === 'proxyImage')) {
            const imageUrl = (req.query.url || req.body?.body?.url);
        ...
        switch (action) {
                ...
                const result = await withSession(async (token) => {
                    ...
                    if (filters) {
                        if (filters.ids && Array.isArray(filters.ids) && filters.ids.length > 0) {
                            query = filters.ids.map((id) => {
                                const q = { 'studio_work_inventory_no_pk': id };
                                if (!ignoreAvailability)
                                    q['Available for sale'] = 'yes';
                                return q;
                            });
                        }
                        else {
                            ...
                            if (filters.title)
                                queryObject['work_name'] = `*${filters.title}*`;
                            if (filters.category)
                                queryObject['work_category'] = filters.category;
                            if (filters.series)
                                queryObject['Work Series'] = filters.series;
                            if (filters.materials)
                                queryObject['materials_for caption'] = `*${filters.materials}*`;
                            ...
                            if (filters.gallery_id)
                                queryObject['gallery_id'] = filters.gallery_id;
                            if (filters.work_id)
                                queryObject['studio_work_inventory_no_pk'] = filters.work_id;
                            if (filters.status)
                                queryObject['edition_catalog::Status'] = filters.status;
                            if (filters.loan_to)
                                queryObject['edition_catalog::loan_to'] = filters.loan_to;
                            // Dimensions Filters
                ...
                const result = await withSession(async (token) => {
                    ...
                    while (keepFetching) {
                        ...
                        records.forEach((record) => {
                            ...
                            if (fd.work_category)
                                rawCategories.push(fd.work_category);
                            if (fd['Work Series'])
                                rawSeries.push(fd['Work Series']);
                        });
            ...
            case 'generate2FA': {
                ...
                const newSecret = new otpauth_1.Secret({ size: 20 });
                const totpGen = new otpauth_1.TOTP({
                    issuer: 'Sigalit Landau Gallery',
            ...
            case 'syncGalleries': {
                ...
                const result = await withSession(async (token) => {
                    ...
                    if (!response.ok)
                        throw new Error(`FM Error ${response.status}: ${await response.text()}`);
                    const data = await response.json();
                    const records = data.response?.data || [];
                    const batch = db.batch();
                    const collections = [];
